...
- **Модульная архитектура**: `setup.sh` является лишь легковесным оркестратором, который загружает и последовательно запускает независимые bash-скрипты для каждого компонента (VPS, 3x, AmneziaWG, AdguardHome, вывод конфигураций). Сами модули лежат в `src/setup/` и могут быть запущены индивидуально для отладки или частичного развертывания.
- `setup.sh` выступает арбитратором среды выполнения, задавая глобальные переменные и вызывая скрипты.
