...
Assert-Match -Text $setup -Pattern 'SCRIPT_VERSION="2\.2\.0"' -Message 'setup.sh must expose the current installer version and bump it with each scripted change.'
...
Assert-Match -Text $setup -Pattern 'FINAL_MODE="direct"' -Message 'setup.sh must persist direct as the final AWG mode.'
Assert-NotMatch -Text $setup -Pattern 'FINAL_MODE="cascade-auto"' -Message 'setup.sh must not persist cascade-auto as the final AWG mode anymore.'
...
Assert-Match -Text $setup -Pattern 'resolve_cascade_upstream_address\(\)' -Message 'setup.sh must resolve cascade upstream hostnames before writing the Xray config so domain-based Reality links do not loop through proxied DNS.'
Assert-Match -Text $setup -Pattern 'CASCADE_ADDRESS_IP' -Message 'setup.sh must keep a resolved cascade upstream IP separate from the original host.'
Assert-Match -Text $setup -Pattern 'adg_http_proxy_rule\["outboundTag"\] = "exit-us"' -Message 'setup.sh must send AdGuardHome upstream proxy traffic through the cascade exit when cascade mode is enabled.'
