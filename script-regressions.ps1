...
Assert-Match -Text $setup -Pattern 'grep "\^AWG_SERVER_PRIV=" "\$CREDS_FILE"' -Message 'setup.sh must load the existing AWG server private key from credentials on non-rotating re-runs.'
Assert-Match -Text $setup -Pattern 'grep "\^AWG_CLIENT_PRIV=" "\$CREDS_FILE"' -Message 'setup.sh must load the existing AWG client private key from credentials on non-rotating re-runs.'
Assert-Match -Text $setup -Pattern 'grep "\^AWG_CLIENT_PSK=" "\$CREDS_FILE"' -Message 'setup.sh must load the existing AWG preshared key from credentials on non-rotating re-runs.'
