...
if [ -f "$SCRIPT_DIR/13-3x-helpers.sh" ]; then
    source "$SCRIPT_DIR/13-3x-helpers.sh"
else
    # Fallback
    remove_legacy_xui() {
    ...
    install_3x_ui_interactive() {
        ...
        THREE_X_UI_INSTALLER_URL=${THREE_X_UI_INSTALLER_URL:-"https://raw.githubusercontent.com/mhsanaei/3x-ui/master/install.sh"}
